...
public class ListSearchAdapter extends RecyclerView.Adapter<ListSearchAdapter.ViewHolder> {
    ...
    public ListSearchAdapter(Context mContext, ArrayList<Sura_Quran> lstSura, int language) {
    ...
    public void searching(ArrayList<Sura_Quran> lstSearching){
        lstSura=lstSearching;
        notifyItemRangeChanged(0,lstSura.size());
        notifyDataSetChanged();
    }
    @NonNull
    @Override
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        int pos = lstSura.get(position).getSuraid() - 1;
        switch (language) {
            case 1:
        ...
        if (lstSura.get(position).getPlace() == 1) {
            holder.item_info_part.setText(QuranRules.ARR_SIZE_PART[pos]+" "+mContext.getResources().getString(R.string.verse)+", Makkia");
        } else {
            holder.item_info_part.setText(QuranRules.ARR_SIZE_PART[pos]+" "+mContext.getResources().getString(R.string.verse)+", Madinia");
        }
    }
